...
3. Use `implementer` for the approved step, `resume-work` for recovery, and `change-check` before commit or manual merge.
4. If root cause is still unconfirmed, switch to `debug-investigation`, keep `debug.md`, and keep `implementer` in minimal experiment mode until the cause is confirmed.
