...
import { createExistingRepo, createTestRepo, makeTempDir, readJson, readText, runGit, runInstall, runRepoScript, runScript, runTool, setGitUser, writeConfig, commitAll } from "./helpers/repo.mjs";
...
test("install.sh bootstraps an existing non-empty repo without adding example app files", async () => {
  ...
  await fs.access(path.join(repoRoot, "scripts", "composer-start.sh"));
  await fs.access(path.join(repoRoot, "scripts", "existing.sh"));
  await fs.access(path.join(repoRoot, "tools", "composer.mjs"));
  await fs.access(path.join(repoRoot, "tools", "existing.mjs"));
  await fs.access(path.join(repoRoot, ".codex-composer.toml"));
  await fs.access(path.join(repoRoot, "README.md"));
  await assert.rejects(fs.access(path.join(repoRoot, "frontend", "src", "App.jsx")));

  const config = await readText(path.join(repoRoot, ".codex-composer.toml"));
...
test("parallel split creates only the B worktree and status points A to the current repo", async () => {
  const repoRoot = await createTestRepo();
  ...
  const statusResult = await runScript("composer-status.sh", ["--repo", repoRoot, "--run", runId]);
  const status = await readJson(path.join(repoRoot, ".codex-composer", "runs", runId, "status.json"));
...
test("repo-local empty template safely downgrades the login request to serial", async () => {
  ...
  await runRepoScript(repoRoot, "composer-start.sh", ["--run", runId, "--requirement", "做一个前后端分离的项目，前端用react，后端用golang,实现登录模块"]);
  await runRepoScript(repoRoot, "composer-checkpoint.sh", [
    "--run",
    runId,
    "--checkpoint",
    "clarify",
